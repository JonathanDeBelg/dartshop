...
export default class MollieApplePayDirect extends Plugin {
    ...
    init() {
        ...
        if (window.ApplePaySession && location.protocol === 'https:') {
            const applePayAvailablePromise = this.isApplePayAvailable();
        ...
        if (applePayButtons.length) {
            applePayButtons.forEach(function (item) {
                ...
                item.addEventListener('click', function (e) {
                    ...
                    const csrfTokens = me._data.form.querySelectorAll('#mollie-apd-csrf input[name="_mollie_csrf_token"]');
                    ...
                    const productId = me._data.form.querySelector('input[name="id"]').value;
                    const productName = me._data.form.querySelector('input[name="name"]').value;
                    const productPrice = me._data.form.querySelector('input[name="price"]').value;
                    const countryCode = me._data.form.querySelector('input[name="countryCode"]').value;
                    const currency = me._data.form.querySelector('input[name="currency"]').value;

                    const productPricePromise = me.getProductPrice(productId);
    ...
    createPaymentRequest(type, countryCode, currencyCode, label) {
        ...
        session.onvalidatemerchant = function (event) {
            const validationPromise = me.performValidation(event.validationURL);
        ...
        session.onshippingcontactselected = function (event) {
            ...
            const shippingMethodsPromise = me.getShippingMethods(countryCode);
            ...
            shippingMethodsPromise
                .then(function (shippingMethods) {
                    if (
                        ...
                        const shippingPromise = me.getShippingAmount();
        ...
        session.onpaymentauthorized = function (event) {
            const paymentPromise = me.sendPaymentToken(event.payment);
    ...
    displayNotification(message, session, type) {
        const flashBagsContainer = document.querySelector('div.flashbags.container');
        ...
        if (flashBagsContainer !== undefined) {
            const html = `<div role="alert" class="alert alert-${type}"><div class="alert-content-container"><div class="alert-content">${message}</div></div></div>`;
